...
export class AlarmsManager {
  ...
  public async getAlarms(myUser: MyUser): Promise<Alarm[]> {
    ...
    const alarmOccurrencesPerAgent = await this.getAlarmOccurrencesPerAgent(
      chunkedAgentIds,
      alarmsPerAgent
    );
    ...
    const agentsOrAssets = apiAgentsAndAssets.map((a) => {
      const agentId =
        "agent" in a && a.agent?.publicId ? a.agent.publicId : a.publicId;
      return new AgentOrAsset(
        a.publicId,
        a.name ?? undefined,
        alarmsPerAgent.find((b) => b.agentId === agentId)?.alarms,
        alarmOccurrencesPerAgent.find(
          (b) => b.agentId === agentId
        )?.alarmOccurrences
      );
    });
  ...
  public getActiveAlarms(myUser: MyUser): Promise<Alarm[]> {
    return this.getAlarms(myUser).then((alarms) =>
      alarms.filter((alarm) => !!alarm.activeOccurrence)
    );
  }

  private filterAlarmsOnAudiences(
    alarms: Alarm[],
    myUserMemberships: MyUserMembership[]
  ): Alarm[] {
    return alarms.filter((alarm) => {
      ...
      const relevantUserMemberships = myUserMemberships.filter((membership) => {
        const sameAudience = membership?.role?.audiences?.find(
          (a) => a.publicId === audienceId
        );
        const sameAgent =
          membership.agents?.find((a) => a?.publicId === agentId) ||
          membership.group?.agent?.publicId === agentId;
        const sameAsset = membership.assets?.find(
          (a) => a?.publicId === agentId
        );
  ...
  private async getAlarmsPerAgent(
    chunkedAgentIds: string[][]
  ): Promise<AlarmsPerAgent> {
    ...
    for (const agentChunk of chunkedAgentIds) {
      const chunkResults = await Promise.all(
        agentChunk.map(async (agentId) => {
          ...
          if (
            alarmsPerAgent.find((a) => a.agentId === agentId)?.alarms.length ===
            0
          ) {
          ...
          return {
            agentId,
            alarmOccurrences:
              await this.apiService.getLatestActiveAlarmOccurrences(agentId),
          };
        })
      );
